...
static void button_clicked(GtkWidget *button, MsgBoxResponse response) {
  action_taken = response;
  exit_popup_mainloop = TRUE;
}

static gboolean timeout_handler(GtkWidget *dlg) {
  action_taken = default_action;
  exit_popup_mainloop = TRUE;
  return FALSE;
}

static void make_button(GtkBox *box, MsgBoxResponse buttons, MsgBoxResponse def,
...
MsgBoxResponse popup_dialog(char *title, MsgBoxResponse buttons,
...
MsgBoxResponse popup_msgbox(char *title, MsgBoxResponse buttons,
				   gint timeout_millis, MsgBoxResponse def,
				   char *format, ...) {
  va_list vl;
